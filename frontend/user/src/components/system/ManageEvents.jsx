...
export default function ManageEvents() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_BASEURL}/events/rooms/user/${userId}`);
    ...
    const handleDelete = async (eventId) => {
        try {
            await axios.delete(`${process.env.NEXT_PUBLIC_BASEURL}/events/${eventId}`);
            toast({
            ...
            toast({
                ...
                description: `${error.message} - At ${new Date()}, our system detected a new request to our server from your IP.`,
            })
        }
    };
                    ...
                    data.map((item, index) => (
                        <div key={index} className='w-full flex flex-col gap-2'>
                            <div className={`h-[150px] w-full rounded-xl ${getRandomGradient()} relative overflow-hidden`}>
                                <div className="absolute top-0 right-0 p-2 cursor-pointer bg-red-50 hover:bg-red-100 text-red-500 rounded-bl-xl z-50" title="Delete" onClick={() => handleDelete(item._id)}>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                    </svg>
                                </div>
                                <Link href={`/dashboard/manage-events/${item._id}`} className="absolute top-[48px] right-0 p-2 cursor-pointer bg-blue-50 hover:bg-blue-100 text-blue-500 rounded-bl-xl rounded-tl-xl z-50" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
