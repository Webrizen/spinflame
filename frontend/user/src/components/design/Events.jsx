...
export default function Events() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BASEURL}/events/rooms`
        );
  ...
  const filteredEvents = data
    ? data
        .filter(
          (event) => !event.winner && new Date(event.startDate) > new Date()
        )
        .filter((event) =>
  ...
  return (
    <section className="w-full min-h-screen pb-16 pt-12">
      <div className="container mx-auto">
        ...
        <div className="grid md:grid-cols-4 grid-cols-1 gap-4 my-4 relative">
          {loading ? (
            ...
            <div className="w-full flex flex-col justify-center items-center text-center absolute top-0">
            <img src="/no-results.svg" alt="No Results" className="aspect-square h-[300px] w-auto" />
